...
if (!defined('IN_ECS'))//检查这个IN_ECS常量是否存在，不存在die掉
{
    die('Hacking attempt');
}
...
if (__FILE__ == '')//当前脚本的路径为空时输出Fatal error code: 0
{
    die('Fatal error code: 0');
}

/* 取得当前ecshop所在的根目录 */
define('ROOT_PATH', str_replace('includes/init.php', '', str_replace('\\', '/', __FILE__)));//定义常量ROOT_PATH为（把绝对路径中的\\替换为/，然后在把这个路径里的includes/init.php替换为空）防止多次引入init.php
